...
def upload():
    ...
    subsectors = json.dumps(subsectors, ensure_ascii=False)
    segments = json.dumps(segments, ensure_ascii=False)
    return render_template('upload_v2.html',sectors=sectors, projectNames=projectnames, datatypes=datatypes,years=years, geographys=geographys,
            ...
            if file_ext not in app.config['UPLOAD_EXTENSIONS']:
                ...
                with open('Metadata_new.json') as json_file:
                    ...
                    metadata_dict = {'Filename': filename, 'Geography': Geography, 'Sector': Sector, 'Sub-Sector': Subsector,
                                     'Segments': Segments, 'Year': Year,'Keyword':keyword,'Project Name':project_name,
                                     'Datatype':datatype,'Partner':Partner_name,'Project Type':Project_type,'Project Lead':Project_Lead,'Project Account':Project_account,
                                     'Team Member':team_member,'Comments':comments,'Uploaded By':user,'Filepath': filepath}
